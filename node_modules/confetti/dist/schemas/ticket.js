import { z } from 'zod';
import { baseFindAllOptionsSchema, staticBaseFindAllOptionsSchema, staticBaseFindOptionsSchema, findBaseOptionsSchema, } from './resource-options.js';
export const TicketSchema = z.object({
    ...
    checkinAt: z.date().describe(JSON.stringify({
        label: 'Checkin At',
    })),
    waitlistAt: z.date().describe(JSON.stringify({
        label: 'Waitlist At',
    })),
    ...
    values: z.looseObject({}).describe(JSON.stringify({
        label: 'Values',
    })),
    comment: z.string().describe(JSON.stringify({
        label: 'Comment',
    })),
    guests: z.number().describe(JSON.stringify({
        label: 'Guests',
    })),
    termsAcceptedAt: z.date().describe(JSON.stringify({
        label: 'Terms Accepted At',
    })),
    deletionRequestedAt: z.date().describe(JSON.stringify({
    ...
    ticketBatchId: z.number().describe(JSON.stringify({
        label: 'Ticket Batch Id',
    })),
    paymentId: z.number().describe(JSON.stringify({
        label: 'Payment Id',
    })),
    eventId: z.number().describe(JSON.stringify({
        label: 'Event Id',
    })),
    contactId: z.number().describe(JSON.stringify({
        label: 'Contact Id',
    })),
});
...
const ticketsFindAllSchema = {
    filter: z.object({
        ...
        status: z
            .union([
            z.array(z.enum(['attending', 'waitlist', 'declined', 'invited', 'consumed', 'deletion-requested']).describe(JSON.stringify({
                label: 'Ticket Status',
    ...
    sort: z
        ...
        .optional(),
    include: z
        .array(z.enum(['addons']).describe(JSON.stringify({
